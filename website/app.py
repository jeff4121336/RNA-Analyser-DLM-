...
def predict():
  ...
  if modelSelected == "m1" or modelSelected == "m2":
    if modelSelected == "m1":
  ...
  elif modelSelected == "m3":
    model = RNAClassifier_2(len(rna_list1), num_channels, kernel_size, dropout_rate, padding).to(device)
    model.load_state_dict(torch.load('./models/m3.pt')['model_state_dict'])
    model.eval()
    ...
    # print(y_pred.shape)
    if y_pred.shape[0] != 1:
      combined_y_pred = torch.sum(y_pred, dim=0)
      combined_y_pred = torch.sigmoid(combined_y_pred)
      resultant_y_pred = combined_y_pred.view(1, y_pred.shape[1], y_pred.shape[2])
    else:
      resultant_y_pred = y_pred
    

    resultant_y_pred = (resultant_y_pred  > 0.3).to(torch.float32)[:,:,:-2]
    # print(resultant_y_pred)
    # print(resultant_y_pred.shape)
    final_output = torch.sum(resultant_y_pred, dim=1)
    ...
    if torch.equal(min_val, max_val):
      if int((max_val.cpu().numpy())[0]) == (1 * resultant_y_pred.shape[1]):
        final_output_normalized = torch.ones(final_output.shape)  # All detected as 1's
      else:
        final_output_normalized = torch.zeros(final_output.shape)  # All detected as 0's
    else:
      final_output_normalized = (final_output - min_val) / (max_val - min_val + epsilon)

    final_output_normalized = final_output_normalized.view(-1)
    final_output_normalized = final_output_normalized.cpu().numpy().tolist()
    
    resultant_y_pred = resultant_y_pred.squeeze(0)
    # print(resultant_y_pred.shape)
    count_of_ones = torch.sum(resultant_y_pred == 1, dim=0)
    # print(count_of_ones)
    max_count, max_index = torch.max(count_of_ones, dim=0)
    thershold = (max_count.item()) // 4
    # print(thershold, final_output_normalized.shape)
  
    result_list = (count_of_ones > thershold)
    print(result_list)
    predicted = '</br>'.join([f"{rna_list1[i]}: {'Detected' if result_list[i] else 'Not Exist'}" for i in range(len(rna_list1) - 2)])
    formatted_scores = '</br>'.join([f"{rna_list1[i]}: {round(score * 100, 3)}"for i, score in enumerate(final_output_normalized)])
